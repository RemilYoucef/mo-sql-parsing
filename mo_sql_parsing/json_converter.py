...
def get_tables(req, alias):
    ...
    if 'where' in req:
        ...
        op = list(v.keys())[0]
        if op in ['and','or']:
            liste_tables += get_op_tables(v[op],alias)[0]
            alias = {**alias, **get_op_tables(v[op], alias)[1]}

        elif isinstance(v[op], dict):
            liste_tables += get_tables(v[op],alias)[0]
...
def get_op_tables(req, alias):
    ...
    for elt in req: 
        ...
        elif isinstance(elt[sub_op], list):
...
def get_tables_fj(req, alias):
    ...
    if 'where' in req:
        ...
        op = list(v.keys())[0]
        if op in ['and','or']:
            liste_tables_from += get_op_tables_fj(v[op],alias)[0]
            liste_tables_join += get_op_tables_fj(v[op],alias)[1]
            alias = {**alias, **get_op_tables_fj(v[op], alias)[2]}

        elif isinstance(v[op], dict):
            liste_tables_from += get_tables_fj(v[op],alias)[0]
            liste_tables_join += get_tables_fj(v[op],alias)[1]
    ...
    if isinstance(v,str):
        ...
        if '.' in v:
            ...
            if alia in alias:
                ...
                table = alias[alia]
                liste_projections += [table + '.' + projection]
            ...
            liste_projections += [alias[v]] if v in alias else [v]
    elif isinstance(v,dict): 
        liste_projections += get_projections_int((list(v.values())[0]), alias)
...
def get_atts_where(req, alias):
    ...
    return get_atts_where_int(req['where'], alias) if 'where' in req else []
...
def get_atts_where_int(v, alias):
    ...
    if isinstance(v,str):
        ...
        if v: 
            if 'where' in v:    
                liste_atts_where += get_atts_where_int(v['where'], alias)
            else:
...
def get_op_atts_order_by(v, alias):
    ...
    if isinstance(v, dict):
        ...
        if v:
            if 'orderby' in v:
                liste_atts_orderby += get_atts_order_by(v, alias)

            else:
                liste_atts_orderby += get_op_atts_order_by((list(v.values())[0]), alias)
    ...
    if isinstance(v,str):
        ...
        if '.' in v:
            ...
            if alia in alias:
                ...
                table = alias[alia]
                full_att = table + '.' + att
                liste_atts_orderby += [full_att]

        else:
            liste_atts_orderby += [v]
    elif isinstance(v,dict):
        liste_atts_orderby += get_atts_orderby_int((list(v.values())[0]), alias)
...
def get_op_atts_group_by(v, alias):
    ...
    if isinstance(v,dict):
        if v:
            if 'groupby' in v:
                liste_atts_groupby += get_atts_group_by(v, alias)

            else:
                liste_atts_groupby += get_op_atts_group_by((list(v.values())[0]), alias)
    ...
    if isinstance(v,str):
        if '.' in v:
            ...
            if alia in alias:
                ...
                table = alias[alia]
                full_att = table + '.' + att
                liste_atts_groupby += [full_att]

        else:
            liste_atts_groupby += [v] 


    elif isinstance(v,dict):
        liste_atts_groupby += get_atts_groupby_int((list(v.values())[0]), alias)
...
def get_op_atts_having(v, alias):
    ...
    if isinstance(v,dict):
        ...
        if v:
            if 'having' in v:
                liste_atts_having += get_atts_having(v, alias)

            else:
                liste_atts_having += get_op_atts_having((list(v.values())[0]), alias)
    ...
    if isinstance(v,str):
        ...
        if '.' in v:
            ...
            if alia in alias:
                ...
                table = alias[alia]
                full_att = table + '.' + att
                liste_atts_having += [full_att]
        else: 
            liste_atts_having += [v]

    elif isinstance(v,dict):
        liste_atts_having += get_atts_having_int((list(v.values())[0]), alias)
...
def get_op_functions(v):
    ...
    if isinstance(v,dict):
        if v:
            if 'select' in v:
                liste_functions += get_functions(v)

            else:
                liste_functions += get_op_functions(list(v.values())[0])
...
def get_functions(req):
    ...
    liste_functions = get_functions_int(req['select']) if 'select' in req else []
    if 'where' in req : 
...
def get_functions_int(v):
    ...
    if isinstance(v,dict):
        ...
        if (list(v.keys())[0]) in functions:
            liste_functions += [list(v.keys())[0]]

        liste_functions += get_functions_int(list(v.values())[0])
